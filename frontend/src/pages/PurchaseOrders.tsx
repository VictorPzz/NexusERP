...
const statusColors: Record<string, string> = {
  ...
  received: 'bg-blue-100 text-blue-800', cancelled: 'bg-red-100 text-red-800',
};
...
export default function PurchaseOrders() {
  ...
  const load = () => {
    setLoading(true);
    api.get('/PurchaseOrders', { params: { page, pageSize: 10 } })
  ...
  const handleSave = async () => {
    ...
    try {
      await api.post('/PurchaseOrders', { supplierId: form.supplierId, notes: form.notes || undefined, details: details.filter((d) => d.productId > 0) });
  ...
  const handleStatus = async () => {
    ...
    try { await api.put(`/PurchaseOrders/${statusModal.id}/status`, { status: newStatus }); setStatusModal(null); load(); } catch { /* ignore */ }
  };
  ...
  const columns = [
    ...
    { key: 'total', label: 'Total', render: (o: PurchaseOrder) => `$${o.total.toLocaleString()}` },
    { key: 'status', label: 'Estado', render: (o: PurchaseOrder) => <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColors[o.status] || 'bg-gray-100'}`}>{o.status}</span> },
    { key: 'actions', label: '', render: (o: PurchaseOrder) => <button onClick={() => { setStatusModal(o); setNewStatus('approved'); }} className="text-blue-600 hover:text-blue-800 text-xs">Estado</button> },
  ];
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nueva Orden de Compra" wide>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select className={select} value={newStatus} onChange={(e) => setNewStatus(e.target.value)}>
              ...
              <option value="received">Recibida</option><option value="cancelled">Cancelada</option>
            </select>
          </div>
