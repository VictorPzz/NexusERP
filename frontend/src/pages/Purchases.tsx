...
export default function Purchases() {
  ...
  const [statusModal, setStatusModal] = useState<Purchase | null>(null);
  const [newStatus, setNewStatus] = useState('confirmed');
  const [form, setForm] = useState({ supplierId: 0, notes: '' });
  ...
  const load = () => {
    setLoading(true);
    api.get('/Purchases', { params: { page, pageSize: 10 } })
      .then((res) => { setPurchases(res.data.data.items || []); setTotalPages(res.data.data.totalPages); })
  ...
  const openCreate = () => { setForm({ supplierId: 0, notes: '' }); setDetails([{ ...emptyDetail }]); setError(''); setModalOpen(true); };
  ...
  const handleSave = async () => {
    ...
    try {
      await api.post('/Purchases', { supplierId: form.supplierId, notes: form.notes || undefined, details: details.filter((d) => d.productId > 0) });
  ...
  const handleStatus = async () => {
    ...
    try { await api.put(`/Purchases/${statusModal.id}/status`, { status: newStatus }); setStatusModal(null); load(); } catch { /* ignore */ }
  };
  ...
  const columns = [
    ...
    { key: 'total', label: 'Total', render: (p: Purchase) => `$${p.total.toLocaleString()}` },
    { key: 'status', label: 'Estado', render: (p: Purchase) => <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColors[p.status] || 'bg-gray-100'}`}>{p.status}</span> },
    { key: 'paymentStatus', label: 'Pago', render: (p: Purchase) => <span className={`px-2 py-1 rounded-full text-xs font-medium ${p.paymentStatus === 'paid' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{p.paymentStatus}</span> },
    { key: 'actions', label: '', render: (p: Purchase) => <button onClick={() => { setStatusModal(p); setNewStatus('confirmed'); }} className="text-blue-600 hover:text-blue-800 text-xs">Estado</button> },
  ];
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nueva Compra" wide>
      ...
      <Modal open={!!statusModal} onClose={() => setStatusModal(null)} title="Cambiar Estado">
        <div className="space-y-4">
          <p className="text-sm text-gray-600">Compra: <strong>{statusModal?.purchaseNumber}</strong></p>
          <div>
